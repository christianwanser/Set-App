# this is a variation of solver_final that outputs images at each step
# of the solving pipeline. Its functionality is identical to solver_final,
# give of take comments/documentation.
import cv2
import numpy as np
import math as m
from random import *
...
def get_card_color(card):
    ...
    # # display the card
    plt.figure()
    plt.imshow(card_saturated, cmap="gray")
    plt.show()
    ...
    # # display the hue pixels
    plt.figure()
    plt.hist(hue_pixels, bins=180)
    plt.show()
...
def get_cards(img):
    ...
    # if len(contours) < 12:
    #     # if fewer than 12 contours, just keep every contour
    #     contours = sorted_contours
    # else:
    #     cnt = sorted(contours, key=cv2.contourArea, reverse=True)[11]
    #     # only keep contours within 10% of the 12th largest contour
    #     # this allows us to include a 13th, 14th, etc. card if it's
    #     # roughly the same size as the other cards
    #     contours = [
    #         c for c in contours if cv2.contourArea(c) > 0.90 * cv2.contourArea(cnt)
    #     ]
    ...
    # plot the boxes on top of the original image
    plt.figure()
    plt.imshow(img)
    for box in boxes:
...
def get_shape_and_num_shapes(card):
    ...
    num_blobs, blob_centers, keypoints = get_blobs(edges_dilate)
    ...
    # plot the filled shapes
    plt.figure()
    for shape in filled_shapes:
        plt.imshow(shape, alpha=0.5, cmap="gray")
    plt.show()
...
def get_card_fill(card):
    ...
    # plot the saturation of masked section of card
